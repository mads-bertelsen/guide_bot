...
% openings. factor from 1 to 5 ?
    ...
    McStasStr.optimvals.min(linp+1)=-1;
    McStasStr.optimvals.max(linp+1)=1;
    McStasStr.optimvals.guess(linp+1)=0;
    ...
    McStasStr.optimvals.min(linp+2)=-1;
    McStasStr.optimvals.max(linp+2)=1;
    McStasStr.optimvals.guess(linp+2)=0;
    ...
    l{end+1} = ['smallaxis_x' num ' = 0.5*startx' num '+ 0.5*fabs(smallaxis_x_factor' num ')*(max_smallaxis_x' num '-startx' num ');'];
    l{end+1} = ['}'];
    l{end+1} = ['else {'];
    %l{end+1} = ['smallaxis_x' num ' = 0.5*endx' num '*smallaxis_x_factor' num ';'];
    l{end+1} = ['smallaxis_x' num ' = 0.5*endx' num '+ 0.5*fabs(smallaxis_x_factor' num ')*(max_smallaxis_x' num '-endx' num ');'];
    l{end+1} = ['}'];
    l{end+1} = ['if (starty' num ' > endy' num ') {'];
    %l{end+1} = ['smallaxis_y' num ' = 0.5*starty' num '*smallaxis_y_factor' num ';'];
    l{end+1} = ['smallaxis_y' num ' = 0.5*starty' num '+ 0.5*fabs(smallaxis_y_factor' num ')*(max_smallaxis_y' num '-starty' num ');'];
    l{end+1} = ['}'];
    l{end+1} = ['else {'];
    %l{end+1} = ['smallaxis_y' num ' = 0.5*endy' num '*smallaxis_y_factor' num ';'];
    l{end+1} = ['smallaxis_y' num ' = 0.5*endy' num '+ 0.5*fabs(smallaxis_y_factor' num ')*(max_smallaxis_y' num '-endy' num ');'];
    l{end+1} = ['}'];
    ...
    l{end+1} = ['// positive smallaxis factor corresponds to smallaxis inside of guide segment'];
    l{end+1} = ['// negative smallaxis factor corresponds to smallaxis outside of guide segment'];
    l{end+1} = ['tmp_w1=startx' num ';'];
    l{end+1} = ['tmp_w2=endx' num ';'];
    l{end+1} = ['tmp_b=smallaxis_x' num ';'];
    l{end+1} = ['tmp_L=length' num ';'];
    l{end+1} = [''];
    l{end+1} = ['if (smallaxis_x_factor' num ' > 0) {'];
    l{end+1} = ['tmp_k=cos(asin(tmp_w2/(2*tmp_b)))/cos(asin(tmp_w1/(2*tmp_b)));'];
    l{end+1} = ['} else {'];
    l{end+1} = ['tmp_k=cos(asin(tmp_w2/(2*tmp_b)))/cos(PI-asin(tmp_w1/(2*tmp_b)));'];
    l{end+1} = ['}'];
    ...
    l{end+1} = ['} else {'];
    l{end+1} = ['tmp_c=cos(PI-asin(tmp_w1/(2*tmp_b)))*cos(PI-asin(tmp_w1/(2*tmp_b)));'];
    l{end+1} = ['}'];
    ...
    l{end+1} = ['tmp_L=length' num ';'];
    l{end+1} = [''];
    l{end+1} = ['if (smallaxis_y_factor' num ' > 0) {'];
    l{end+1} = ['  tmp_k=cos(asin(tmp_w2/(2*tmp_b)))/cos(asin(tmp_w1/(2*tmp_b)));'];
    l{end+1} = ['} else {'];
    l{end+1} = ['  tmp_k=cos(asin(tmp_w2/(2*tmp_b)))/cos(PI-asin(tmp_w1/(2*tmp_b)));'];
    l{end+1} = ['}'];
