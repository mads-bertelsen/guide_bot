function [McStasStr] = Cg_module(McStasStr,index,last,requirements,demands,options,globalinfo,ghost_globalinfo,defaults)
...
if (index==1 && index==last)
    disp('ERROR, Cg (curved guide) can not be the only component!')
elseif (index==1)
    disp('ERROR, Cg (curved guide) can not be the last component!')
elseif (index==last)
    disp('ERROR, Cg (curved guide) can not be the first component!')
else
...
for ops=1:length(options)
    ...
    elseif (strcmp(options{ops}(1:equals-1),'start') || strcmp(options{ops}(1:equals-1),'length') || strcmp(options{ops}(1:equals-1),'changed') ||...
       ...
       disp(['ERROR, the input ' options{ops} ' is not valid in module Cg (Curved guide)'])
    end
...
l{1}=['COMPONENT Curved_guide_gravity_' num '= Elliptic_guide_gravity('];
l{2}=['xwidth=startx' num ',yheight=starty' num ',l=length' num ','];
l{3}=['dimensionsAt="mid",curvature=curve_radius' num ','];
l{4}=['linxw=500,linyh=500,loutxw=500,loutyh=500,'];
l{5}=['R0=R0' num ', Qc=Qc' num ', alpha=alpha' num ', m=m' num ', W=W' num ')'];
l{6}='AT (0,0, u) RELATIVE PREVIOUS';    
if globalinfo.rotsign(index)>0    
l{7}='ROTATED (0,0,0) RELATIVE PREVIOUS'; % Bend -x direction
else
l{7}='ROTATED (0,0,180) RELATIVE PREVIOUS'; % Bend x direction
end
l{8}='';
l{9}=['COMPONENT EndOfelement_' num '= Arm()'];
l{10}=['AT (0,0,length' num ' + 2*u) RELATIVE PREVIOUS'];
if globalinfo.rotsign(index)>0    
...
l{1}=['COMPONENT Curved_guide_gravity_' num '= Elliptic_guide_gravity('];
l{2}=['xwidth=starty' num ',yheight=startx' num ',l=length' num ','];
l{3}=['dimensionsAt="mid",curvature=curve_radius' num ','];
l{4}=['linxw=500,linyh=500,loutxw=500,loutyh=500,'];
l{5}=['R0=R0' num ', Qc=Qc' num ', alpha=alpha' num ', m=m' num ', W=W' num ')'];
l{6}='AT (0,0, u) RELATIVE PREVIOUS';    
if globalinfo.rotsign(index)>0    
l{7}='ROTATED (0,0,90) RELATIVE PREVIOUS'; % Bend -y direction
else
l{7}='ROTATED (0,0,-90) RELATIVE PREVIOUS'; % Bend y direction
end
l{8}='';
...
if (index == last)
    disp('ERROR, module Cg (curved guide) can not start a guide');
elseif (index == 1)
    disp('ERROR, module Cg (curved guide) can not end a guide');
else
